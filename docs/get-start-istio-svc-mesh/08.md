...
在前一章中，我们使用了 Istio 提供的一些可观察性特性。我们能够捕获应用程序指标，但是指标只是可观察性的一个方面。可观察性是指在所有可能的维度上收集数据。在应用程序停机期间，查看可观察性的各个方面有助于开发人员理解应用程序行为，以便执行事件分析。有许多工具可以帮助实现这一目标。但是，了解添加一个行为所需的成本是很重要的。运营团队应该能够配置他们选择的工具，而不需要开发人员。在本章中，我们将看到如何无缝地捕获额外的行为，如请求跟踪和应用程序日志。我们还将使用 Istio 即插即用模型，该模型为捕获额外的应用程序行为提供了统一的机制。

## 分布式跟踪

在微服务应用程序中，一个请求通常部分由部署在集群中的多个应用程序提供服务。分布式跟踪是跨不同应用程序跟踪请求流的过程。跟踪通常通过显示某个时刻的请求、响应和延迟来描述应用程序的行为。运营团队通常使用跟踪来确定哪些服务导致了应用程序的性能问题。分布式跟踪有很多解决方案，比如 Zipkin、Jagger、Skywalking 等等。Istio 服务网格可以与它们一起工作。跟踪通常由特使代理生成。然后，这些跟踪被发送到跟踪器后端。
...
在我们继续之前，我们需要将一个 tracer 应用程序部署到我们的 Kubernetes 集群。在这一章中，我们将使用 Jagger，一个在优步开发的开源分布式追踪应用程序。Jagger 基于 Dapper 和 OpenZipkin 提出的概念。出于我们的目的，我们将使用 Jagger 操作符( [`https://github.com/jaegertracing/jaeger-operator`](https://github.com/jaegertracing/jaeger-operator) )部署 Jagger。Kubernetes 有一个操作扩展，可以用来在 Kubernetes 集群上打包和部署应用程序。第一步，我们需要通过执行以下命令来安装 Jagger 操作符:
...
在此之后，Envoy 将生成请求跨度，并将它们发送到 Jagger 服务器。我们可以通过执行前端 Java 应用程序的请求来验证这一点。
...
现在让我们查找 Jagger UI 并搜索以前执行的请求。所有请求对于前端和 webapp 应用程序都有一个跨度。参见图 [8-2](#Fig2) 。
...
如图 [8-2](#Fig2) 所示，Jagger 提供了所有已执行请求的直方图。它显示了处理请求所花费的时间以及调用的应用程序。我们可以单击各个跟踪，详细查看各个应用程序的延迟和时间线。

## 应用程序日志

Istio 不为管理 Kubernetes 集群中运行的应用程序生成的日志提供任何支持。这在大型部署中是一个巨大的挑战，因为为我们的每个应用程序生成的日志都保留在运行该应用程序的容器中。让我们回头看看我们在第 [3](03.html) 章开发的例子。我们用 Java 创建了一个前端应用程序，用 Python 创建了一个 web 服务后端。我们在集群中部署了两个后端实例和一个前端应用程序实例。我们可以对我们的前端执行请求，这将调用后端。为了调试行为，我们需要查看应用程序日志。这是通过使用以下命令对每个容器执行日志查找来实现的:
...
这是非常麻烦和容易出错的，因为我们需要查找每个容器。此外，随着容器重启，我们会丢失应用程序日志中的信息。日志记录包含最详细的系统状态，团队必须能够参考日志来跟踪、验证和诊断应用程序的状态。因此，我们可以说应用程序日志的处理不够好，我们需要一个更好的解决方案来做到这一点。

除了应用程序日志之外，应用程序还可以创建访问日志。传统上，我们在前端代理中看到过这种情况，Apache HTTP 服务器正在创建`access.log`文件。日志记录包含我们的应用程序收到的请求以及对它的响应。这是非常有用的信息。现在，如果我们看一下 Istio 服务网格，所有请求都是通过 Envoy sidecar 发出的。sidecar 因此跟踪它接收到的请求-响应。我们可以配置 Envoy 代理来打印这些日志或创建一个日志文件。但这对我们没有帮助，因为容器重启会丢失所有这些信息。

Kubernetes 描述了一种集群级日志记录方法，该方法利用了 ELK、Splunk、Stackdriver 等日志记录后端应用程序。该方法利用了服务网格使用的 sidecar 模式。应用程序日志的完整解决方案如下所示:
...
下面的`fluentd.conf`文件读取我们的应用程序生成的日志，并将它们发送到 ELK 聚合器:
...
如果前面的配置有效，我们将在 ELK 堆栈中看到我们的应用程序日志。现在下一步是将 Istio 生成的访问日志发送到 ELK 实例。在下一节中，我们将看看 Mixer 扩展，它可以用来发送所需的日志。

## 搅拌器

Istio 使用可扩展的混合器子系统捕获所有遥测数据。该子系统非常灵活，允许对不同的监控和警报系统采用即插即用的方法。这种抽象使运营团队能够改变他们的应用程序监控方法，而不需要任何开发更改。Istio Mixer 部署了许多适配器。这些适配器中的每一个都向诸如 Prometheus、StatsD 等监控系统提交所需的数据。Envoy sidecar 为每个请求调用 Mixer，从而通过适配器捕获所有数据。由于 Envoy 为它收到的每个请求调用 Mixer，因此将 Mixer 组件嵌入到 sidecar 中听起来可能是合乎逻辑的。但是具有独立混合器组件的方法具有以下优点:
...
Istio 提供了多种适配器，可以使用 Mixer 进行配置。我们可以尝试扩展上一节中的日志示例。我们能够在 ELK 实例中发送我们的应用程序日志。我们现在需要发送访问日志。现在让我们试着用混音器来实现这一点。在我们继续之前，调用以下命令来获取可用适配器的列表:
...
既然我们知道有各种适配器可用，我们将尝试为我们的应用程序配置它们。每个可用的适配器都可以使用图 [8-4](#Fig4) 所示的组件进行配置。
...
注意，我们没有描述日志格式，我们为应用程序日志描述了日志格式。
...
在这一章中，我们看了 Istio 提供的可观察性特性。我们从捕获请求跟踪开始，以确定应用程序性能。Istio 允许我们使用自己选择的跟踪解决方案。接下来，我们希望捕获应用程序级日志。我们意识到 Istio 没有提供应用程序日志的解决方案。然而，我们可以扩展我们的 Kubernetes 集群，为我们部署的应用程序提供一个日志解决方案。该解决方案不需要任何额外的开发工作。为此，我们部署了一个 ELK 实例，并使用 Sidecar 模式路由应用程序日志。下一个目标是扩展日志解决方案，以包含由 Istio 生成的日志。在旅途中，我们使用混合器组件来实现边车日志摄取。总之，我们使用了 Istio 的可扩展性特性，该特性可用于将其与第三方系统连接。