...
在大多数生产集群中，网关与 Kubernetes 负载均衡器服务一起配置。在这种情况下，Kubernetes 服务会创建一个基于云的 L4 负载均衡器。负载均衡器有一个公共 IP 地址，Kubernetes 集群外部的世界可以访问该地址。当负载均衡器收到请求时，它会将请求委派给匹配的 Istio 网关。然后，网关使用 Istio 流量路由，并将请求分派给适当的服务版本。Istio 还将必要的遥测技术和安全性应用于网关。见图 [5-1](#Fig1) 。
...
现在我们需要测试配置。为此，我们需要负载均衡器的地址。执行以下命令确定地址(参见图 [5-2](#Fig2) ):
