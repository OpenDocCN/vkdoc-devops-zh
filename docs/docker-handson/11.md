...
服务发现是基于微服务的体系结构中的关键组成部分。服务发现是知道进程何时监听运行特定 TCP 或 UDP 端口的应用进程并使用名称连接到这些进程的过程。

在现代云基础设施中，每个应用都应该针对故障进行设计。因此，web 服务器、数据库和应用服务器的多个实例将会运行，并且它们使用 API、消息队列等相互交互。任何服务都可能在给定的时间点失败，并横向扩展。当这些新的服务实例出现时，它应该能够向基础设施中的其他组件宣传自己。这就是领事介入的地方。
...
Consul 有自己的键/值存储。应用可以使用其键/值存储来进行诸如领导者选举、协调和标记等操作。所有的操作都可以使用 API 调用来执行。

多数据中心

您可以为 consul 配置多个数据中心。因此，如果您的应用衍生到多个区域，您不需要为多区域支持创建另一个抽象层。
...
现在我们有了一个具有自动注册容器服务的集群。接下来，我们将研究如何发现其他应用容器要使用的注册服务。可以使用 DNS 或 HTTP API 发现 consul 中的服务。

http 请求示例:

使用 http 请求，您可以获得服务细节并在您的应用上使用它。运行以下命令来获取我们之前部署的数据库服务的详细信息。
...
Apache ZooKeeper 是一个集中式配置管理器，由分布式应用(如 Mesos)用来协调集群中的活动。Mesos 使用 ZooKeeper 来选举一个主节点，并让奴隶加入集群。
...
就其本身而言，Mesos 只提供集群的基本“内核”层。它允许其他应用请求集群中的资源来执行任务，但自己不做任何事情。

框架在 Mesos 层和您的应用之间架起了一座桥梁。它们是更高级别的抽象，简化了在集群上启动任务的过程。
...
Marathon 相当于 Linux upstart 或 init 守护进程，是为长时间运行的应用设计的。您可以使用它来启动、停止和扩展集群中的应用。
...
上图显示了 Mesos 的主要组件。Mesos 由一个主守护进程和 mesos 应用(也称为框架)组成，主守护进程管理运行在每个集群节点上的从守护进程，而 mesos 应用在这些从守护进程上运行任务。

主服务器通过向应用提供资源来实现资源(CPU、RAM)的细粒度共享。每个资源提议包含来自单个从设备的资源列表。主节点根据给定的组织策略，如公平共享或严格的优先级，决定向每个框架提供多少资源。为了支持一组不同的策略，master 采用了模块化的体系结构，使得通过插件机制添加新的分配模块变得容易。
...
Mesosphere 是一个基于 Apache Mesos 的软件解决方案。使用 mesosphere，您可以使用 Apache Mesos 的所有功能和附加组件来管理基础设施。对于扩展应用，您可以通过消除许多与应用扩展相关的挑战，将 marathon 和 chromos 之类的框架用于 mesosphere。以下是中间层提供的主要特性
...
成功执行上述命令后，您可以在 marathon 控制台中看到正在部署的应用。

![](../images/00267.jpeg)

向上扩展码头集装箱:

使用 marathon 可以非常容易地扩展 Docker 实例。单击已部署的应用并选择 scale 选项。给它一个数字，例如:3，然后单击确定。
...
Kubernetes 是 Google 开发的集群管理工具，用于管理容器化的应用。您可以让一堆节点看起来像一台大型计算机，并将容器应用部署到您的公共云和私有云。它将离散节点抽象化并优化计算资源。

Kubernetes 使用声明性方法来获取用户提到的应用的期望状态。当应用部署在 kubernetes 集群上时，kubernetes 主节点决定应用必须部署在哪个底层主机上。Kubernetes 调度程序完成应用部署的工作。此外，kubernetes 的自我修复、自动重启、复制和重新调度机制使其更加健壮，更适合基于容器的应用。

Kubernetes 由两个主要组件组成，一个主服务器和一个附属服务器。让我们看一下这两个组件以及与每个组件相关联的服务。

主服务器

主服务器是 kubernetes 的控制单元。它充当用户在集群上部署应用的主要管理组件。主服务器由用于调度、通信等操作的各种组件组成。
...
pod 是部署在同一主机上的一组相关容器。pod 中的一个或多个容器被视为一个应用。例如，一群 web 服务器可以被分组到一个 pod 中。豆荚共享相同的环境，被视为一个整体。分组到 pod 中的应用可以共享卷、IP 空间，并且可以作为单个单元进行扩展。

服务

服务为 kubernetes 集群中运行的应用提供发现能力。服务更像是一个命名的负载平衡器，充当一组容器的接口。您可以创建服务单元，它知道所有支持的服务。它充当应用的单一访问点。例如，所有 web 服务器容器都可以使用单个访问点来访问应用容器。通过这种机制，您可以扩大和缩小应用容器，而访问点保持不变。
...
使用 Docker 在 kubernetes 集群上部署多层应用

在本例中，将使用预配置的 Docker 容器部署多层留言簿应用(前端、redis 从和主)。

用于部署该应用的 pod 描述文件(JSON 文件)位于/kubernetes/examples/guestbook 目录下的 kubernetes 源文件中。
