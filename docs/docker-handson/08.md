...
当一个存储库被成功推送时，webhooks 将自动触发其他应用的基于 REST 的操作。当一个 webhook 被调用时，它将生成一个带有 JSON 有效负载的 HTTP POST 方法。
...
在这一章中，我们将从头开始学习如何对应用进行分类。您可以在您的应用中使用 Docker 公共注册表中的图像，但是为了深入了解 dockerizing 应用，我们将从头开始构建我们的图像。

在本节中，我们将创建以下内容

1.  运行在 apache 上的静态 web 应用
2.  一张 MySQL 图片
3.  带有 MySQL 数据库的 WordPress 应用
...
ENV 设置环境变量，并接受一个键值对作为参数。ENV 指令设置的变量可以被容器内部运行的脚本和应用使用。docker 文件中的这一功能为在 docker 容器中运行程序提供了更好的灵活性。
...
使用此指令，可以将特定的应用设置为默认应用，并在每次使用映像创建容器时启动。
...
ENTRYPOINT 可以与 CMD 一起使用，从 CMD 中删除“应用”,只留下将传递给 entry point 的参数。
...
*   每个容器只运行一个进程。为了更好的水平伸缩和容器重用，将应用解耦是一个很好的实践。例如，在不同的容器中运行 web 应用和数据库，并使用"- -link "标志将它们链接在一起。
