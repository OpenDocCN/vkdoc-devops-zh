...
现在我们的服务器中有了一个工作的 Docker 应用。在进入更多 Docker 操作之前，让我们先来看看 Docker hub。
...
我们通过启动一个没有任何应用的容器来测试 Docker 安装。Docker 的主要目标是在容器内部运行应用。用于运行容器的命令是“Docker run”。它采用各种参数，如图像、命令等。
...
正如你在上面的图片中看到的，它将当前的工作目录显示为根目录，并列出根目录下的所有目录。你可以在容器中安装应用和创建目录，就像在任何普通的 Ubuntu 机器上一样。
...
当您退出容器时，交互式容器停止。这对运行应用没有帮助，因为您希望机器保持运行。

要让容器处于运行模式，我们必须将其妖魔化。在下一节中，我们将看看如何妖魔化一个容器。

可以使用“-d”标志创建一个后台化的容器。这就是我们创建 Docker 应用的方式。以下命令将在守护模式下创建一个容器。
...
到目前为止，本章我们已经学习了如何使用 Docker 命令以及如何获得每个命令的帮助。在下一节中，我们将学习如何在容器中运行一个基本的 python 应用。

Python web 应用容器:

我们之前创建的容器只是在其中运行了一个 shell 脚本。在本节中，我们将学习如何在容器中运行 python flask 应用。

我们将使用来自 Docker hub 的预配置映像(training/webapp ),其中配置了 python flask 应用。让我们从 Docker run 命令开始。
...
3.  –P:该标志用于将主机端口映射到容器端口，以便访问容器应用。由于我们没有指定任何端口号，Docker 将用随机端口映射主机和容器。
4.  training/webapp:这是包含 python flask 应用的图像名称。Docker 从 Docker hub 下载这个图像。
5.  Python app.py:这是创建容器后将在容器内部执行的命令。它启动容器内的 python 应用。
...
让我们尝试从浏览器访问端口 49153 上的 python 应用。
...
您将在浏览器中看到一个示例 hello world 应用。

也可以手动配置端口映射。让我们用手动配置的端口创建同一个 python 应用容器。运行以下命令启动容器，将主机端口 5000 映射到容器端口 5000。

sudo dock run-d-p 5000:5000 training/web app python app . py

![](../images/00069.jpeg)

现在，如果您从浏览器访问端口 5000 上的 python 应用，您将看到相同的应用。这就是 Docker 没有一对一端口映射的优势。

因此，您可以在不同的端口上运行应用的多个实例。
...
现在我们有了一个工作的 python 应用，我们可以执行下面的操作来获得关于容器的更多信息。
...
现在，如果您执行“docker ps -l”命令，您可以看到启动的容器，或者您可以在浏览器中查看应用。
