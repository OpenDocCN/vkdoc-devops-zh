...
你一直在使用容器与 Keycloak 和 Kafka 等应用进行交互，但你也在第 [1](1.html) 章中使用了容器来运行骆驼路线。至此，您已经很好地理解了容器的含义及其提供的好处。让我们从 Kubernetes 的高级架构开始，重点关注它的工作原理以及它提供的自动化类型。看图 [6-1](#Fig1) 。每个盒子代表一台主机。
