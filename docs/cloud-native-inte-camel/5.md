...
Kafka 的信息处理有些不同。主题分为**个分区**。每个分区都是存储主题消息的独立结构。为了实现这个解释，想象一个有两个代理(Kafka 实例)的 Kafka 集群，如图 [5-4](#Fig4) 所示。
...
继续我们对 Kafka 描述中“分布式”含义的转换，你需要理解如何组装一个集群。看一下图 [5-5](#Fig5) 。
...
查找类似清单 [5-2](#PC4) 的日志条目。如果你找到了他们，这意味着 Kafka 已经准备好了。
...
这个项目只有两个依赖项，`camel-quarkus-rest`和`camel-quarkus-kafka`。你已经知道的 REST 组件。它将负责添加 webserver 实现并启用 REST DSL。Kafka 成分是你的研究对象。让我们看看这个项目路线是什么样子的。打开清单 [5-4](#PC16) 所示的`RestKafkaRoute.java`文件。
