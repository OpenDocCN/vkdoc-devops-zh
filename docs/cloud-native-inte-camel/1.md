...
在本书的大部分时间里，我将重点关注集成案例和解决这些案例的技术，但所有决策都将考虑云原生应用程序的最佳实践。在您对集成技术和模式有了坚实的理解之后，在最后一章中，您将深入了解如何在 Kubernetes 中部署和配置开发的应用程序。
...
此后，`Hello World`应用程序应该开始运行。查找如下所示的日志条目:
