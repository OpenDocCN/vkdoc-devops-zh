...
| 规格->选择器->类型 | 服务类型。 | LoadBalancer(负载平衡器) | 不 |
...
| 规格->模板->规格->容器->图像 | 码头工人形象 | svds/人权中心 |
...
在本节中，我们将在交互式 shell 中运行一个示例 MapReduce 应用程序。`hdfs`命令用于运行 MapReduce 应用程序。在交互式 shell 中调用`hdfs`命令。
...
接下来，我们将运行一个`wordcount`应用程序。我们将从`/input`目录文件中获取输入，并在`/output`目录中输出。创建`/input`目录并将其权限设置为全局(777)。
...
接下来，使用下面的命令运行`wordcount`示例应用程序，其中包含示例应用程序的 jar 文件由 jar 参数指定，并且`/input`和`/output`目录分别被设置为输入目录和输出目录的最后两个命令参数。
...
MapReduce 作业完成后运行`wordcount`应用程序。图 [10-22](#Fig22) 中显示的是`wordcount` MapReduce 作业的输出，而不是字数统计结果。
...
服务在服务端点公开由复制控制器管理的 pod，服务端点只是外部客户机可以调用应用程序的主机:端口设置。运行以下命令创建服务。
...
在本章中，我们使用 Kubernetes 集群管理器根据 Docker 映像`svds/cdh`创建了一个 Pods 集群。我们使用声明式和命令式方法来创建集群。我们使用 kubectl scale 命令来扩展集群。我们还演示了如何使用打包在`cdh`映像中的一些 Apache Hadoop 框架。我们运行了一个 MapReduce `wordcount`示例应用程序。我们还运行了 Apache Hive 和 Apache HBase 工具。在下一章，我们将讨论在索引和存储框架 Apache Solr 中使用 Kubernetes。