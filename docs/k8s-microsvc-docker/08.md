...
输出有关 Pod 的详细信息，如名称、名称空间、映像、节点、标签、状态、IP 地址和事件，如图 [8-16](#Fig16) 所示。Pod 标签是复制控制器定义文件中指定的`app=cassandra`。
...
所有正在运行的容器被列出，如图 [8-17](#Fig17) 所示。复制`cassandra`映像容器的容器 id。
...
按照前面对定义文件的讨论，创建一个复制控制器。应该创建一个 Pod。列出Docker容器。
...
复制映像“cassandra”的 Docker 容器的容器 id，如图 [8-31](#Fig31) 所示。
