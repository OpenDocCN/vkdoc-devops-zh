...
| 种类 | 定义文件的种类。 | POD |
| 元数据 | Pod 元数据。 |   |
| 元数据->标签 | POD标签。服务选择器利用标签来选择要管理的 pod。 | app:couch app |
| 元数据->名称 | Pod 名称。 | 美洲狮号 |
| 投机 | Pod 规格。 |   |
| 规格->容器 | POD里的容器。 |   |
...
| 规格->模板->规格->容器->映像 | Docker的形象。 | 美洲狮号 | 是 |
...
接下来，我们将启动一个交互式 bash shell 来从命令行访问 Couchbase 服务器。根据 Docker 映像“couchbase”获取其中一个 Docker 容器的容器 id，如图 [9-38](#Fig38) 所示。
...
一个单元被删除，但单元总数为 1，低于配置的副本数。因此，复制控制器启动一个新的副本。随后列出POD。
