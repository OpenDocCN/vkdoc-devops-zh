...
Pod 内的所有容器共享相同的端口空间。同样，尽管指定端口号(并命名它们)不是强制性的，但只要声明了两个或更多端口，就必须命名端口。此外，当端口在 pod 清单上正式声明时，服务公开(第四章 ??)需要的步骤更少。底线是声明网络端口是良好的 Pod 清单卫生，所以让我们在下面名为`nginx-port.yaml:`的清单示例中看看如何做
