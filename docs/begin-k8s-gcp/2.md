...
Pod 清单是以声明方式描述 Pod 属性的文件。所有的POD都被公式化为一个对象结构。每当我们使用诸如`kubectl run`这样的命令时，我们实际上是在动态地创建一个 Pod 清单。事实上，我们可以通过向大多数命令添加`--dry-run`和`-o yaml`标志来查看结果清单。例如:
...
正如所料，`/tmp/log.txt`只显示了日期时间戳的一个实例，而`/data/log.txt`显示了三个实例，尽管容器已经崩溃了三次。这是因为，如前所述，`emptyDir`与POD的生命周期息息相关。事实上，删除并重启 Pod 将会删除`emptyDir`，因此在重启 Pod 后立即查询`/data/log.txt`将只显示一个条目:
...
这些都是普通的POD。我们可以对它们运行我们目前所学的所有命令。我们只需要记住给我们使用的每个命令添加`-n kube-system`标志。否则 Kubernetes 会假设`-n default`。例如，让我们看看在名为`kube-dns-*-p95tp`的 Pod 中找到的第一个容器内部正在运行什么进程:
