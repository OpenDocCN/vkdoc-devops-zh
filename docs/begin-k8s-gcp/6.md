...
在`Never`和`OnFailure`的`restartPolicy`值之间做出决定，取决于我们更能接受什么样的妥协。`Never`通常是最明智的选择，因为它不处理故障吊舱的输出，并允许我们排除故障；然而；将失效的 Pods 留在周围会占用更多的资源。理想情况下，工业作业解决方案应该将有价值的数据保存到永久存储介质中，例如第 [2](2.html) 章中演示的附件。
