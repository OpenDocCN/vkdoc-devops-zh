...
服务发现是必要的，因为 pod 是易变的；在它们的生命周期中，它们可能会被创建和销毁多次，每次都会获得不同的 IP 地址。Kubernetes 的自我修复和扩展特性也意味着我们通常需要虚拟 IP 地址和循环负载均衡机制，而不是特定的、类似宠物的 pod 的离散地址。
...
在本章中，我们了解到服务控制器有助于在服务消费者和 pod 之间创建一个间接层，以便于工具化属性，如自我修复、金丝雀释放、负载均衡、正常启动和关闭以及零停机部署。
