...
Apache Hadoop 有几个 Docker 映像。我们使用了 Docker Hub 上的`sequenceiq/hadoop-docker` Docker 图像。下载标签为 2.7.0 的 Docker 图像或最新的标签图像(如果不同)。
...
Docker 图像`sequenceiq/hadoop-docker`被下载，如图 [8-3](#Fig3) 所示。
...
在本节中，我们将为 MapReduce Word Count 应用程序创建输入文件，该应用程序包含在 Hadoop 发行版打包的示例中。要创建输入文件，请将目录(cd)更改为`$HADOOP_PREFIX`目录。
...
在本节中，我们将运行一个 MapReduce 应用程序进行字数统计；应用程序打包在`hadoop-mapreduce-examples-2.7.0.jar`文件中，可以用参数“wordcount”调用。`wordcount`应用程序要求提供输入和输出目录。输入目录是我们之前创建的 HDFS 中的`/input`目录，输出目录是`/output`，它在运行 hadoop 命令之前必须不存在。从交互式 shell 中运行下面的`hadoop`命令。
...
纱线作业完成，如图 [8-14](#Fig14) 所示，字数统计应用程序输出到 HDFS 的`/output`目录。
...
列出两个文件:`_SUCCESS`，表示纱线作业成功完成，`part-r-00000`，是`wordcount`应用程序的输出，如图 [8-15](#Fig15) 所示。
...
随后运行`docker ps`命令，没有集装箱被列为运行中，如图 [8-17](#Fig17) 所示。
...
在本章中，我们在 Docker 容器中运行了 Apache Hadoop 组件。我们创建了一些文件，并把这些文件放在 HDFS。随后，我们运行了一个与 Hadoop 发行版中的示例打包在一起的 MapReduce `wordcount`应用程序。我们还介绍了基于 Cloudera Hadoop 发行版(CDH)的 Docker 映像，我们还将在基于 Apache Hadoop 生态系统框架的后续章节中使用它。