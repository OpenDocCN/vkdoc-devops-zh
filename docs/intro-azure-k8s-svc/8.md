...
当代码被提交到存储库时，它可能会引发一系列事件。使用我们的 **iaks** 图表示例，该图表利用了容器图像 *iaks/azure-voting-app。*当开发人员提交构建该映像的`Dockerfile`的新版本时，可能会触发一个流水线。一旦图像被更新并通过流水线中的必要测试， **iaks** Helm图可以用新的图像版本进行测试。假设所有的测试都通过了， **iaks** Helm chart 可以被更新以使用新的图像作为`values.yaml`文件中的默认标签。
