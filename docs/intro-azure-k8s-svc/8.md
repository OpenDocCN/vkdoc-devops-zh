...
清单 [8-11](#PC11) 显示了 Helm 图表的标准文件和文件夹结构。所需文件以**粗体**显示。
...
为了从 Helm 杆上永久移除释放，必须使用清单 [8-40](#PC40) 中所示的`--purge`标志。
...
Helm 图可以用几种方法测试。可以使用`helm lint`命令运行一个测试来验证图表在语法上是正确的。清单 [8-41](#PC41) 显示了对照 **iaks** 图表运行`helm lint`的输出。
