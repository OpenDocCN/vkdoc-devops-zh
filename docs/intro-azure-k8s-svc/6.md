# 6.通过 Azure Kubernetes 服务部署和使用 Rancher
...
*   Rancher 简化舵图 : Rancher 允许你加载舵图库和/或 Rancher 库。这些库使得轻松地部署应用成为可能。
...
回到 Rancher 门户，您现在可以单击 AKS 集群来访问仪表板、监控和集群设置，从 Rancher 或 Helm 目录安装应用，启动 Kubectl 等。图 [6-10](#Fig10) 是 Rancher 中 AKS 集群的仪表板示例。
