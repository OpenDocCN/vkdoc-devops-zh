...
pod 是 Kubernetes 中的一个或多个容器，它们共享资源并耦合在一起。它代表一个部署单元。pod 封装了一个应用，包括容器、存储、网络 IP 以及如何运行容器的配置。把POD想象成包裹容器的包装纸。pod 通常部署成两种模式之一:第一种模式是运行单个容器的 pod，第二种模式是运行需要紧密耦合在一起的多个容器的 pod。
