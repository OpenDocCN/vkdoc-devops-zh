...
本章讨论了使用 Sematext SPM 性能监控和 Logsene 日志管理对 Docker 群进行连续日志记录和监控。首先，您学习了如何创建 SPM 应用和 Logsene 应用。然后，您在每个群节点上安装了 Sematext 代理服务，并在 SPM 仪表板中监控指标和事件。您还了解了如何在 Logsene UI 或 Kibana 4 仪表板中监控日志。下一章将讨论 Docker 集群中的负载平衡。