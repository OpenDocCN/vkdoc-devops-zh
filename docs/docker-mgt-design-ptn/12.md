...
AWS 弹性负载平衡提供两种类型的负载平衡器——经典负载平衡器和应用负载平衡器。传统负载平衡器基于应用或网络级别的信息路由流量，而应用负载平衡器基于高级应用级别的信息路由流量。经典的负载均衡器应该能够满足对多个 EC2 实例的流量进行最简单的负载均衡，并且是我们用于 Docker Swarm 实例的负载均衡器。选择经典负载平衡器，然后点击继续，如图 [12-9](#Fig9) 所示。
...
更新 listeners 选项卡中的负载平衡器侦听器，添加/修改一个侦听器，其负载平衡器端口设置为 8080，实例端口设置为 8080，这是我们创建的 Hello World 服务的发布端口，如图 [12-24](#Fig24) 所示。
