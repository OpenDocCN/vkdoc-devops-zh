...
网站的高可用性是指网站持续可用，没有服务中断。通过在 Docker Swarm 应用中提供容错功能，网站变得高度可用。高可用性在不同的级别上提供。入口负载平衡器在多个服务任务之间平衡传入的客户端请求，并在任务级别提供容错。如果一个服务任务失败，客户端流量将被路由到另一个服务任务。为跨多个可用性区域托管的 Docker 群使用外部负载平衡器是提供高可用性的另一种方法。外部负载平衡器在节点级别提供容错。如果一个节点出现故障，客户端流量将被路由到另一个节点上的群节点。
