...
Docker Swarm 模式是 Docker 1.12 版的 Docker 原生模式，用于为开发 Docker 应用程序创建分布式和可扩展的服务。

## 问题

虽然单个 Docker 映像应用程序也很常用，但绝大多数 Docker 企业应用程序都由多个映像组成，这些映像之间存在依赖关系。Docker Compose(在 v1 和 v2 中是独立的)可以通过使用`links`和`depends_on`选项来声明微服务之间的依赖关系，但是 Compose(独立的)是过时的，不同于在群模式服务的上下文中定义服务的格式。

## 解决方案

Docker Swarm mode 引入了服务栈来定义服务的集合(Swarm mode services ),这些服务彼此自动链接，以提供服务之间存在依赖关系的逻辑分组。堆栈使用的堆栈文件是 YAML 文件，其格式非常类似于`docker-compose.yml`格式。有一些不同之处，比如缺少了用于定义 Docker Compose (standalone)中微服务之间依赖关系的`links`和`depends_on`选项。YAML ( [`http://www.yaml.org/`](http://www.yaml.org/) )是配置文件常用的一种数据序列化格式。

从 Docker v1.13 开始，已经引入了命令的`docker stack`子集来创建 Docker 堆栈。使用一个定义多个服务的堆栈文件，包括服务的配置，如环境变量、标签、容器数量和卷，一个`docker stack deploy`命令创建一个服务堆栈，如图 [15-1](#Fig1) 所示。这些服务会自动相互链接。
...
Docker Compose v3.x 及更高版本完全兼容 Docker Swarm 模式，这意味着 Docker Compose v3.x `docker-compose.yml`文件可以用作堆栈文件，但堆栈文件中不支持的几个子选项(包括`build`、`container_name`、`external_links`和`links`)除外。Docker Compose 3.x 仍然可以独立使用来开发非群模式服务，但这些微服务无法与 Docker 群模式`docker service`命令组一起使用或扩展。

要使用堆栈来管理群模式服务，必须满足以下要求。

*   Docker 版本必须是 1.13 或更高版本
*   必须启用群组模式
*   堆栈文件 YAML 格式必须基于 Docker Compose v3.x 文件格式

要使用服务堆栈，可以使用 Docker Compose 第 3 版 YAML 文件格式，但不需要安装 Docker Compose。
...
| 命令 | 描述 |
| `deploy` | 部署服务堆栈或更新现有堆栈 |
| `ls` | 列出堆栈 |
| `ps` | 列出堆栈中的群组模式任务 |
| `rm` | 移除堆栈 |
| `services` | 列出堆栈中的群组模式服务 |

运行`docker --version`命令列出 Docker 版本。要列出堆栈使用的命令，请运行`docker stack`命令。
...
2.  启用群体模式。
3.  使用 Docker Compose(3 . x 版)YAML 格式创建一个堆栈文件。
4.  使用`docker stack`组命令创建和管理堆栈。
...
如第 [3](03.html) 章所述，使用创建堆栈向导配置一个群组。您可以指定群组管理器的数量为 1、3 或 5，群组工作节点的数量为 1-1000。我们使用了一个群管理器节点和两个群工作者节点，如图 [15-3](#Fig3) 所示。
...
使用用于创建云形成堆栈的密钥对 SSH 登录到 Swarm manager 实例。
...
要创建由两个服务组成的服务堆栈，一个用于 WordPress 博客，另一个用于 MySQL 数据库，使用 Docker Compose 版本 3 YAML 格式( [`https://docs.docker.com/compose/compose-file/`](https://docs.docker.com/compose/compose-file/) )创建一个堆栈文件。创建一个`docker-cloud.yml`堆栈文件(文件名是任意的)来分别使用 Docker 映像`wordpress`和`mysql`指定两个服务(`web`和`mysql`)。为 Docker 图像设置环境变量。唯一需要设置的环境变量是`mysql` Docker 图像的`MYSQL_ROOT_PASSWORD`。`wordpress` Docker 图像的`WORDPRESS_DB_PASSWORD`环境变量默认为`MYSQL_ROOT_PASSWORD`，但也可以明确设置为与`MYSQL_ROOT_PASSWORD`相同的值。表 [15-2](#Tab2) 中列出了`wordpress` Docker 图像使用的其他一些环境变量。
...
接下来，将 Docker 映像和环境变量指定到堆栈文件中，以创建服务堆栈。要将 Docker Compose YAML 文件格式用于群组模式堆栈，请将堆栈文件中的`version`指定为`3`或更高版本，如`3.1`。列出了`docker-cloud.yml`文件:
...
`8080:80`的`ports`映射将 WordPress Docker 容器端口 80 映射到主机端口 8080。创建堆栈时，会忽略任何堆栈文件选项，例如前面列表中包含的`docker stack deploy`不支持的链接。将前面的清单作为`docker-cloud.yml`存储在 Swarm manager EC2 实例中。在 Swarm manager 中列出文件应该会列出`docker-cloud.yml`文件。
...
配置了包含两个服务的堆栈文件后，接下来我们将创建一个服务堆栈。

## 创建堆栈

`docker stack deploy`命令用于创建和部署堆栈。它具有以下语法。
...
| [计]选项 | 描述 | 缺省值 |
| --- | --- | --- |
| `--bundle-file` | 分布式应用程序包文件的路径。从 Docker 合成文件创建应用程序包，就像从 Docker 文件创建 Docker 映像一样。应用程序包可用于创建堆栈。在开发本章时，应用程序包是一个试验性的特性，不在本章中讨论。 |   |
| `--compose-file, -c` | 堆栈文件的路径。 |   |
| `--with-registry-` `auth` | 是否向群代理发送注册认证信息。 | 错误的 |

使用堆栈文件`docker-cloud.yml`，用`docker stack deploy`命令创建一个名为`mysql`的 Docker 堆栈。
...
会创建一个 Docker 堆栈，并忽略群组模式中不支持的`links`选项。除了网络`mysql_default`之外，还创建了两个群服务`mysql_mysql`和`mysql_web`。
...
使用`docker stack services`命令列出`mysql`堆栈中的服务，其语法如下。
...
将列出过滤后的堆栈服务。因为两个服务都是使用`–filter`指定的，所以两个服务都被列出。
...
`docker stack ps`命令用于列出堆栈中的 Docker 容器，其语法如下:使用`--help`选项输出命令用法。
...
要列出`mysql`堆栈中的所有 Docker 容器，运行以下命令。
...
默认情况下，为每个服务创建一个副本，因此为堆栈中的每个服务列出一个 Docker 容器。两个 Docker 容器都运行在一个 Swarm worker 节点上。
...
接下来，我们使用这个栈来创建一个 WordPress 博客。可以在 Swarm manager 主机的端口`8080`上访问名为`web`的堆栈服务。获取群管理器节点 EC2 实例的公共 DNS，如图 [15-8](#Fig8) 所示。
...
`docker stack rm STACK`命令用于移除堆栈。使用以下命令移除`mysql`堆栈。
...
删除了`mysql`堆栈，并且`docker stack service mysql`命令没有列出堆栈，如该命令的输出所示。
...
本章介绍了 Docker 1.13 中添加的 Docker 原生特性 stacks。堆栈是相关服务的集合，使用堆栈文件创建，堆栈文件以类似于 Docker Compose v3.x YAML 语法的 YAML 格式定义。本章总结了这本书关于 Docker 管理设计模式的内容。随着 Docker 中新特性的加入，其他设计模式也可以用于开发 Docker 原生应用程序。