...
| [计]选项 | 描述 | 缺省值 |
| --- | --- | --- |
| `--bundle-file` | 分布式应用包文件的路径。从 Docker 合成文件创建应用包，就像从 Docker 文件创建 Docker 映像一样。应用包可用于创建堆栈。在开发本章时，应用包是一个试验性的特性，不在本章中讨论。 |   |
...
本章介绍了 Docker 1.13 中添加的 Docker 原生特性 stacks。堆栈是相关服务的集合，使用堆栈文件创建，堆栈文件以类似于 Docker Compose v3.x YAML 语法的 YAML 格式定义。本章总结了这本书关于 Docker 管理设计模式的内容。随着 Docker 中新特性的加入，其他设计模式也可以用于开发 Docker 原生应用。