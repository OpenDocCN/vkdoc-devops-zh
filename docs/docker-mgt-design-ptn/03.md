...
在指定详细信息中，指定栈名称(`DockerSwarm`)。群组参数部分包含表 [3-1](#Tab1) 中列出的字段。
...
群管理器数量保持默认设置 3，群工作器节点数量保持默认设置 5，如图 [3-6](#Fig6) 所示。
...
![A454123_1_En_3_Fig8_HTML.jpg](../Images/A454123_1_En_3_Fig8_HTML.jpg)
...
接下来，指定栈的选项。可以为栈中的资源指定标签(键值对)。对于权限，可以选择云形成的 IAM 角色。这些选项都不需要设置，如图 [3-9](#Fig9) 所示。
...
开始创建新的栈。点击刷新按钮刷新列表中的栈，如图 [3-13](#Fig13) 所示。
...
如图 [3-14](#Fig14) 所示的状态`CREATE_IN_PROGRESS`所示，基于 Docker Swarm 的云形成模板的新栈开始创建。
...
为不同的栈详细信息提供了不同的选项卡。资源页签显示了 CloudFormation 模板创建的 AWS 资源，如图 [3-15](#Fig15) 所示。
...
事件选项卡显示了创建云形成栈时发生的事件，如图 [3-16](#Fig16) 所示。
...
栈创建完成后，状态显示为`CREATE_COMPLETE`，如图 [3-17](#Fig17) 所示。
...
通过将实例状态设置为 Running，可以列出所有 EC2 实例。列出了 Docker Swarm manager 节点(三个)和 worker 节点(五个)，如图 [3-23](#Fig23) 所示。管理节点和工作节点位于三个不同的可用性区域。
...
要删除一个蜂群，从云编队控制台选择操作➤删除栈，如图 [3-27](#Fig28) 所示。
...
在删除栈确认对话框中，点击是，删除，如图 [3-28](#Fig29) 所示。
...
栈状态变为`DELETE_IN_PROGRESS`，如图 [3-29](#Fig30) 所示。
...
随着每个栈的资源被删除，其状态变为`DELETE_COMPLETE`，如图 [3-30](#Fig31) 中事件选项卡上的一些资源所示。
