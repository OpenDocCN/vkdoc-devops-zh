...
MQTT <sup>1</sup> (消息队列遥测传输)是一种发布/订阅消息协议，专为物联网和 IIoT(工业物联网)实现而设计。设备可以通过 MQTT 代理发布和订阅主题，代理可以被桥接在一起。MQTT 被设计成轻量级的，能够在资源受限的环境中运行，包括 Raspberry Pi。<sup>22T7】</sup>
