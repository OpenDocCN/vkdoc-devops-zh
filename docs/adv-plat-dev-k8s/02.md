...
在私有注册表中构建和托管容器可以确保对映像的更大信任。但是，从基础映像构建新映像是很常见的，包括 Ubuntu、Alpine 或 CentOS 等发行版。几乎所有软件最终都包含常见漏洞和暴露(CVE)。用于检测 CVEs 和其他安全风险的解决方案，例如 CoreOS 的 Clair、 <sup>[9](#Fn9)</sup> 可以直接集成到构建流水线中。
