...
Kubernetes 节点角色是一个以`node-role.kubernetes.io/<role>`开头的简单标签。Pod 调度程序是由 Kubernetes 设计的，用于找到 Pod 的最佳位置。POD是短暂的；它们可能会失败或从一个节点耗尽，并随时被重新调度到另一个节点。但是，有时特殊的工作负载需要额外的调度要求。在本章定义的`hc1`集群中，三个节点具有独特的角色`sensor`。角色标签可以是反映其操作特征的任何值。
...
第 6 章[介绍了 JupyterHub 作为一种为用户提供 Jupyter 容器的方式。将 JupyterHub 安装到新的`hc1`集群中(在本章前面已经设置好了)，并为`values.yml`(用于配置Helm图)更新了`singleuser`部分。用清单](06.html) [11-9](#PC51) 的内容替换`singleuser`部分。
