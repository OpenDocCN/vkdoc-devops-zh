...
*   三个 Raspberry Pi 设备(版本 3 或 4)，运行 Raspberry Pi OS Lite (32 位)minimal，并基于 Debian Buster<sup>1T3】</sup>
...
由于在主要框架和软件库中对 CUDA <sup>8</sup> 的广泛支持，NVIDIA GPUs 在机器学习项目中很受欢迎。CUDA(计算统一设备架构)是英伟达的专有 API，仅受英伟达 GPU 支持。CUDA 的开放替代方案是 OpenCL<sup>T5 9T7】在 NVIDIA 和 AMD GPUs 上都支持。然而，截至 2020 年，数据科学生态系统中对 open OpenCL 的支持有限，并且没有最流行的框架 TensorFlow 和 PyTorch 的支持。 <sup>10</sup></sup>
