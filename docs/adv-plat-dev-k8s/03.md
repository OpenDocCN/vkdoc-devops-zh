...
Ubuntu 操作系统使用 Systemd <sup>13</sup> 跟踪使用 Linux cgroups、<sup>T5】14</sup>的进程，默认情况下 Docker 使用 cgroupfs。拥有单独的 cgroup 管理器会在管理负载资源时导致不稳定。通过提供一个配置文件来配置 Docker 使用 Systemd。 <sup>15</sup>
