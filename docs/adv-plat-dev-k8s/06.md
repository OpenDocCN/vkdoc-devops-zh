...
*零到 JupyterHub*<sup>[22](#Fn22)</sup>带 Kubernetes 是一个稳定的 JupyterHub 掌 Helm 图，文档全面详细。尽管为了学习和清晰起见，本书的大部分内容选择使用普通的 YAML 文件，但是 JupyterHub 是一个复杂的系统，这个图表很好地抽象了它，同时提供了任何必要的配置覆盖。
...
3.  在`hub`部分，`extraConfig`用于注入 Helm 表没有直接暴露的附加配置。在这种情况下，配置指示 KubeSpawner <sup>[27](#Fn27)</sup> 在`data-lab`名称空间中生成 JupyterLab Pods，并配置为使用本章前面定义的`data-lab` ServiceAccount。
