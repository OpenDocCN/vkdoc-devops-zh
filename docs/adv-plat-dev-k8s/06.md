...
MQTT 是物联网通信和指标收集的流行选择。第 5 章[介绍了 Mosquitto 为数据流水线提供 MQTT 支持。图](05.html) [6-12](#Fig12) 只描绘了需要从 MQTT 主题中消费事件的几行代码。这个例子显示了从 MQTT.fx 应用程序发送到`dev/apk8s/lightbulb`主题的测试 JSON 消息，如图 [6-11](#Fig11) 所示，运行在本地工作站上。命令`kubectl port-forward svc/mqtt 1883:1883 -n data`允许 MQTT.fx 从本地端口连接到集群。
