...
*   *注册表*是存储图像的地方。我们可以识别两种类型的注册中心:私有的和公共的。可以通过地址 [`https://hub.docker.com`](https://hub.docker.com) 联系公共注册表。这是官方的 Docker Hub 注册表。在这里，我们可以创建一个帐户，并开始在注册表中存储我们的图像。
