...
导出器帮助从应用/Kubernetes 服务获取状态/日志/指标，并向 Prometheus 提供数据。它们类似于市场上其他监控工具中的适配器或插件。Prometheus 提供了一份官方和外部捐助的出口商名单。让我们通过访问下面的: [`https://prometheus.io/docs/instrumenting/exporters/`](https://prometheus.io/docs/instrumenting/exporters/) 来探索一些对集装箱基础设施监控有用的方法。
