...
*   **第二步:**在Prometheus下执行以下命令，克隆 GCP，其中包含`sysdig-agent-clusterrole.yaml`、`sysdig-agent-configmap.yaml`、`sysdig-agent-daemonset-v2.yaml`文件:。`$ git clone` [`https://github.com/dryice-devops/GCP.git`](https://github.com/dryice-devops/GCP.git)
...
*   在`sysdig-agent-configmap.yaml`文件中，你会发现下面这段:`k8s_cluster_name`。传递 GCP EKS 星团(在我们的例子中是Prometheus)的值，如图 [8-23](#Fig23) 所示。
