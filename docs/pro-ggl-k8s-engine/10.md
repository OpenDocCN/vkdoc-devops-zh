...
在本节中，我们将使用`gcloud`来创建/删除 GKE 集群和防火墙规则。Sysdig 将用于监控容器生态系统。为了协调所有这些步骤，我们将使用Jenkins。
...
*   **第五步**:使用从“安装Jenkins”部分的第三步“解锁Jenkins”页面获取的 secrets 密码，点击继续按钮，如图 [10-16](#Fig45) 所示。
...
*   **步骤 2** :导航到前面步骤中用于访问 Jenkins 的 Jenkins URL。使用您在前面步骤中设置的管理员密码。导航到管理Jenkins➤管理节点➤新节点，如图 [10-22.1](#Fig52) 和 [10-22.2](#Fig53) 所示。
...
*   **步骤 5** :为了验证代理是否已连接并正在运行，导航到Jenkins➤节点，然后单击 devopsbox。
