...
当您将代码提交到云源代码库、GitHub 或 Bitbucket 时，它会自动构建映像并将其推送到私有注册表中。用户可以轻松设置 CI/CD 流水线，集成到云构建或直接部署到 Google Kubernetes 引擎、App Engine、云功能或 Firebase。容器登记支持 Docker 图像清单 V2 和 OCI 图像格式。
